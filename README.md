# elipey

```text
> developer / builder / night operator
> web / automation / experiments
```

I turn ideas into working systems.

Currently building with `JavaScript`, `TypeScript`, `React` and `Node.js`.

### projects

- [doce-colheita](https://github.com/elipey/doce-colheita-geleias-artesanais) — responsive storefront built with TypeScript
- [discord-sales-bot](https://github.com/elipey/BOT-DE-VENDAS-PRA-DISCORD) — sales automation with Pix and role delivery

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=elipey&show_icons=true&hide_title=true&hide_border=true&bg_color=00000000&text_color=8b949e&icon_color=39ff14&ring_color=39ff14" height="135" alt="GitHub stats" />
</p>

